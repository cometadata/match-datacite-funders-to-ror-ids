...
CLI tool to extract unique funder names from the DataCite public data file, match them against ROR IDs via a match service, and reconcile matches back to DOI/funder records. Funder matching is performed via the funder matching strategy developed by [Jason Portenoy](https://github.com/h1-the-swan), itself a modified form of the single search match strategy developed by [Dominika Tkaczyk](https://github.com/dtkaczyk). The full matching service codebase available at https://gitlab.com/crossref/marple
...
- A running [marple](https://gitlab.com/crossref/marple) match service instance exposing `GET /match?task=funder&input=<name>`
